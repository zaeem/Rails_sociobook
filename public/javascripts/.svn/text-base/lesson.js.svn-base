 ...
 *
 * Copyright (c) 2006 - 2008 JÃ¶rn Zaefferer
 *
...
/*!
 * application.js
 * http://www.memofire.com
 *
 * Date: Thu Mar 24 11:11 2011 -0300
 */
$(function() {

  /*- Small Code for Browser Detection -*/
  var a = 0;
  ...
  /*- Fallback for the JS -*/
  $('#welcome_login_form #login_form').hide();

  $('a[href="/login"]').click(function(){
    $(this).toggleClass('open');
    var login_height            = $('#login_form').height();
  ...
  $('.lessons-content table tr td').click(function(){
    if ($(this).parent('tr').attr('rel') != null) {
      window.location.href=$(this).parent('tr').attr('rel');
    }
  });
  ...
  $('.trigger').click(function(e){
    ...
    if (!$(this).hasClass('current')) {
      ...
      $('.trigger.current').removeClass('current');
      $(this).addClass('current');

      $('.accordion-container.current').slideToggle(function(){
        $(this).removeClass('current');
      });
      $($(this).attr('href')).slideToggle().addClass('current');
    } else {
      $(this).removeClass('current');
      $('.accordion-container.current').slideToggle(function(){
        $(this).removeClass('current');
  ...
  $('.question_input.expand').focus();
  $.each($('.question_input.expand:visible'), function(index, value) {
    $(value).trigger(e);
  });

  $('.definition.expand').focus();
  $.each($('.definition.expand:visible'), function(index, value) {
    $(value).trigger(e);
  });

  $('.expand.success').focus();
  $.each($('.expand.success:visible'), function(index, value) {
    $(value).trigger(e);
  });

  $('.expand.error.hyphenate').focus();
  $.each($('.expand.error.hyphenate:visible'), function(index, value) {
    $(value).trigger(e);
  });

  $('form#new_answer p.question #answer_answer').focus();

  $('button#proceed_link_btn.btn.right').focus().addClass('orange');

  $('#answer_submit.btn[type="submit"]').blur(function(){
  ...
  $('textarea.question_input').focus();

  $('table#lessons_index_table a').hide();
  $('table#lessons_index_table a').css('margin-left','30px');
  $('table#lessons_index_table a').css('margin-top','6px');
  $('table#lessons_index_table a').css('line-height','10px');
  $('table#lessons_index_table a').css('float','right');
    
  $('table#queue_index_table a').hide();
  $('table#queue_index_table a').css('margin-left','20px');
  $('table#queue_index_table a').addClass('orange-annotate btn');

  $('table#lessons_index_table tr').click(function(event){
    if (event.target.nodeName == 'TD'){
      window.location.href = $(this).attr('id');
    }
  });
  $('table#queue_index_table tr').click(function(event){
    if (event.target.nodeName == 'TD'){
      window.location.href = $(this).attr('id');
    }
  });

  $('table#lessons_index_table tr').mouseover(function(){
    jQuery("a", this).show();
  });
  $('table#lessons_index_table tr').mouseout(function(){
    jQuery("a", this).hide();
  });
  $('table#queue_index_table tr').mouseover(function(){
    jQuery("a", this).show();
  });
  $('table#queue_index_table tr').mouseout(function(){
    jQuery("a", this).hide();
  });

    // anotate interface
//    $('ol.flashcards li').mouseover(function(){
//        alert('imhere');
//        if ($('.list-flashcards', this).siblings().css('display') == 'none'){
//            $('.list-flashcards', this).css('color','#074874');
//        }else{
//            $('.list-flashcards', this).css('color','white');
//        }
//    });
